    ...
    return {
        ...
        createAjaxRequest: function(args) {
            if (args.module) {
                ...
                if (args.variables)
                    args.variables = dq.StringUtil.urlSplit(args.variables);
                if (args.postVariables)
                    args.postVariables = dq.StringUtil.urlSplit(args.postVariables);
...
dq.ajaxParsing = function(el) {

    var attr = el.getAttr("data-ajax-bind");
    var object = dq.StringUtil.objectParsing(attr);
    return object;
}

dq.uiParsing = function(el) {
    var attr = el.getAttr("data-ui-bind")[0];
    var object = dq.StringUtil.objectParsing(attr);
    return object;
}

dq.customParsing = function(el, custom) {
...
dq.Parser = {
    ...
    isUIValid: function(el, component) {
        ...
        if (element.getAttr(component).length == 0)
            return false;
        return element;
    },
